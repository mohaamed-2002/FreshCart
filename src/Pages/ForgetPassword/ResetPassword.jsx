...
export default function ResetPassword() {
  ...
  const validationSchema = Yup.object({
    ...
    newPassword: Yup.string().required().matches(/^[A-Za-z1-9]{8,12}$/, 'password not valid'),
  })
  ...
  return (
    <>
      <section className=" dark:bg-gray-900 w-full md:w-1/2 mx-auto bg-gray-50 shadow p-3 my-3">
        <h1 className="text-3xl font-bold my-3">Reset Your Account</h1>
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="mb-5">
            ...
            <input onChange={formik.handleChange} type="password" onBlur={formik.handleBlur} id="newPassword" name="newPassword" value={formik.values.newPassword} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter new password" />
            {(formik.touched.newPassword && formik.errors.newPassword) && (<small className="text-red-600">{formik.errors.newPassword}</small>)}
          </div>
          ...
          ) : (<button type="submit" disabled={(!formik.isValid)} className="btn">Reset Password</button>
          )}
        </form>
